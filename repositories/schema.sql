...
CREATE TABLE IF NOT EXISTS artwork_explanations (
    artwork_id VARCHAR(36) PRIMARY KEY,
    explanation_xml TEXT NOT NULL,
    image_path VARCHAR(500),  -- Path to image in Supabase Storage
    creator_user_id VARCHAR(36),  -- User who created/uploaded the artwork (nullable for anonymous)
    created_at TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP
);

-- Table for storing subject expansions
CREATE TABLE IF NOT EXISTS subject_expansions (
    expansion_id VARCHAR(36) PRIMARY KEY,
    artwork_id VARCHAR(36) NOT NULL,
    subject VARCHAR(500) NOT NULL,
    expansion_xml TEXT NOT NULL,
    parent_expansion_id VARCHAR(36),
    created_at TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP,
    FOREIGN KEY (artwork_id) REFERENCES artwork_explanations(artwork_id),
    FOREIGN KEY (parent_expansion_id) REFERENCES subject_expansions(expansion_id)
);
